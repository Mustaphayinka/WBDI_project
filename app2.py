...
# Sort the DATA_OPTIONS list alphabetically by the "name" attribute
sorted_data_options = sorted(DATA_OPTIONS, key=lambda x: x["name"])

# Create a Python file (names.py) and write the sorted list to it
with open("names.py", "w") as file:
    file.write("DATA_OPTIONS = [\n")
    for data_option in sorted_data_options:
        file.write(f'    {data_option},\n')
    file.write("]\n")

print("Data sorted and saved to names.py")
